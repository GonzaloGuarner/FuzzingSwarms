...
fisin = readfis('FuzzyInferenceSystems\Compound\FridayAgents.fis');
...
options.MethodOptions.MaxIterations = 55;
options.MethodOptions.SwarmSize = 50;


costFunction = @(fis)simulationCost(fis,fisObst,detectors,mapMatrix,areaPoints,env);
...
function cost = simulationCost(fisAgents,fisObst,detectors,mapMatrix,areaPoints,env)
tic
sumCost = 0;
...
for idx = 1:200%400 %(time units)
    ...
    poses = poses + vel;
    env.Poses = poses;
    sumCost = sumCost + idx*circlesSmart(poses(1,:),poses(2,:),3*ones(1,numRobots), areaPoints, 10);    
end

collisions = CheckObstacles(mapMatrix,numRobots,poses); 
cost = (collisions+1)*100/(sumCost)*(200*201)/2; % 2/(t*(t+1)) * sum_i(t_i * area)
toc
end
