...
swarmAgentsFIS = readfis('FuzzyInferenceSystems\Compound\ThursdayAgentsOut.fis');
...
suma = zeros(totalIterations,totalRuns);
...
for idx = 1:totalIterations%400 %(time units)
    ...
    for rIdx = 1:numRobots
       ...
       detections(:,1) = DetectionNoise(0.5,detections(:,1),R_c);
       end
    ...
    xlim([5 237]);   % Without this, axis resizing can slow things down
    ylim([5 237]); 
...
function detectionRange = DetectionNoise(deviation,detectionRange,maxRange)
    errorr = deviation*randn;
    %error = 0.1*deviation;
    %detectionRange = detectionRange + error;
    detectionRange = detectionRange + errorr;
