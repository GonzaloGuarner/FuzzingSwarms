%% Basic algorithm. Comparison over two arenas of big size x,y in [0 249]
% 
...
mapMatrix = table2array(readtable('Arenas\ArenaOADVFA3.csv'));
env.givenMap = occupancyMap(mapMatrix);
areaPoints = getAreaPoints(mapMatrix);


%% Obtain Fuzzy Inference System
swarmAgentsFIS = readfis('FuzzyInferenceSystems\Compound\ComparisonTFLC\TFLC_OADVFA.fis');
swarmAgentsObst = readfis('FuzzyInferenceSystems\FuzzyAgentsObst3.fis');


%% Initialize robots at random positions
 %poses(1:3,:) = rand(3,numRobots).*[238;238;pi]+[6;6;0];
% poses(1:3,:) = rand(3,numRobots).*[47;47;pi]+[6;188;0];
...
distance = zeros(totalIterations,1);
...
for idx = 1:totalIterations%400 %(time units)
    ...
    for rIdx = 1:numRobots
       ...
       %distance(idx) = distance(idx) + speed;
    end
    poses= poses + vel;
    ...
    xlim([2 244]);   % Without this, axis resizing can slow things down
    ylim([2 244]); 
